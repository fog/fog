  ...
  module AWS
    class Glacier
      ...
      class Job < Fog::Model
        ...
        def save
          requires :vault, :type
          specification = {'Type' => type, 'ArchiveId' => archive_id, 'Format' => format, 'Description' => description, 'SNSTopic' => sns_topic}.reject { |_k,v| v.nil? }
        ...
        def get_output(options={})
          if io = options.delete(:io)
            options = options.merge :response_block => lambda { |chunk, _remaining_bytes, _total_bytes| io.write chunk }
          end
