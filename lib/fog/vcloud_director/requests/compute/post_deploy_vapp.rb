      ...
      class Real
        ...
        def post_deploy_vapp(id, options={})
          body = Nokogiri::XML::Builder.new do
            ...
            attr_reader[:deploymentLeaseSeconds] = options[:deploymentLeaseSeconds] if options.key?(:deploymentLeaseSeconds)
            attr_reader[:forceCustomization] = options[:forceCustomization] if options.key?(:forceCustomization)
            attr_reader[:powerOn] = options[:powerOn] if options.key?(:powerOn)
