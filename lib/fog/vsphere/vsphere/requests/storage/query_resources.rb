    ...
    class Vsphere
      ...
      class Real
        ...
        def fetch_host_storage_resource(options = {})
          ...
          @clusters.each do |cs_mob_ref|
            ...
            host_mob_refs.each do |host_mob_ref|
              ...
              Fog::Logger.debug ("[#{Time.now.rfc2822}] Fog: host_resource.name = #{host_resource.name}")
              host_resource.connection_state = host_mob_ref.summary.runtime.connectionState
              host_datastores = host_mob_ref.datastore
              Fog::Logger.debug ( "[#{Time.now.rfc2822}] Fog: host_datastores size = #{host_datastores.size}")
              host_resource.share_datastores = fetch_datastores(host_datastores,
                                                                @share_datastore_pattern, true)
              Fog::Logger.warning("[#{Time.now.rfc2822}] WARNING: no matched sharestores in host:#{host_resource.name}") if host_resource.share_datastores.empty?
              host_resource.local_datastores = fetch_datastores(host_datastores,
                                                                @local_datastore_pattern, false)
              Fog::Logger.warning("[#{Time.now.rfc2822}] WARNING: no matched localstores in host:#{host_resource.name}") if host_resource.local_datastores.empty?
          ...
          Fog::Logger.warning( "[#{Time.now.rfc2822}] WARNING: total host number = #{@host_list.size}")
          @host_list
        end
        ...
        def fetch_datastores(datastore_mobs, match_patterns, type)
          ...
          Fog::Logger.debug("[#{Time.now.rfc2822}] Fog: datastores length = #{datastores.size}")
          datastores
        end
