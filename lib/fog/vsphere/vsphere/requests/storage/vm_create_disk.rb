    ...
    class Vsphere
      ...
      module Shared
        ...
        def create_scsi_controller(vm_mob_ref)
          begin
            ...
            Fog::Logger.debug("[#{Time.now.rfc2822}] Fog:create_disk error #{e} happened")
          end
          return key
        end
      ...
      class Real
        ...
        def vm_create_disk (options = {})
          ...
          Fog::Logger.debug("[#{Time.now.rfc2822}] Fog: create disk transport options['transport'] = #{options['transport']} fullpath is #{options['vmdk_path']} unit_number=#{options['unit_number']}}")
          if options['transport'] == 'paravirtual'
            controller_key = create_scsi_controller(vm_mob_ref)
            Fog::Logger.debug("[#{Time.now.rfc2822}] Fog: transport controller_key = #{controller_key}[/]")
          else
            Fog::Logger.debug("[#{Time.now.rfc2822}] Fog: transport system_disk[0].controllerKey == #{system_disk[0].controllerKey}[/]")
