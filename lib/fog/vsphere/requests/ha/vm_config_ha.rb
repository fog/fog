      ...
      class Real
          ...
          devices.each do |vm_device|
            if vm_device.kind_of?(RbVmomi::VIM::VirtualCdrom)
              ...
              if need_change
                begin
                  ...
                  Fog::Logger.warning("throw away RbVmomi::VIM::Fault #{e}")
                  response = {
                      'task_state' => 'error',
                      'errors' => e
                  }
            ...
            if vm_device.kind_of?(RbVmomi::VIM::VirtualDisk)
              ...
              if vm_device.backing.thinProvisioned
                Fog::Logger.debug("disk.backing.thinProvisioned = true for device #{vm_device}")
        ...
        def vm_disable_ft(options = {})
          ...
          if ft_info.kind_of?(RbVmomi::VIM::FaultToleranceSecondaryConfigInfo)
            Fog::Logger.debug("RbVmomi::VIM::FaultToleranceSecondaryConfigInfo = #{ft_info.primaryVM}")
            vm_mob_ref = ft_info.primaryVM
          else
            Fog::Logger.debug("RbVmomi::VIM::FaultTolerancePrimaryConfigInfo = #{ft_info.secondaries}")
          end
          begin
