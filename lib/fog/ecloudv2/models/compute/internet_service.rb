module Fog
  module Compute
    class Ecloudv2
      class InternetService < Fog::Ecloudv2::Model
        identity :href
        ...
        def nodes
          @nodes ||= Fog::Compute::Ecloudv2::Nodes.new(:connection => connection, :href => href)
        end

        def monitors
          @monitors ||= Fog::Compute::Ecloudv2::Monitors.new(:connection => connection, :href => "/cloudapi/ecloud/internetServices/#{id}/monitor") 
        end
        ...
        def create_monitor(options = {})
          ...
          monitor = Fog::Compute::Ecloudv2::Monitors.new(:connection => connection, :href => data[:href])
        end

        def disable_monitor
          data = connection.monitors_disable(href + "/action/disableMonitor").body
          task = Fog::Compute::Ecloudv2::Tasks.new(:connection => connection, :href => data[:href])
        end
