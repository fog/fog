...
if options[:methods]
  methods = options[:methods].map do |m|
    m = <<METHOD
def #{m[0]}
          @#{m[0]} = Fog::Compute::Ecloudv2::#{m[0].camelize}.new(:connection => connection, :href => "#{m[1]}")
        end
METHOD
  end.join("\n        ")
end
...
require 'fog/ecloudv2/models/compute/#{options[:model]}'

module Fog
  module Compute
    class Ecloudv2
      class #{options[:collection].camelize} < Fog::Ecloudv2::Collection

        identity :href

        model Fog::Compute::Ecloudv2::#{options[:model].camelize}
