Shindo.tests('Fog::Storage[:openstack] | large object requests', ["openstack"]) do
  ...
  tests('success') do
    ...
    tests("#delete_object('foglargeobjecttests', 'fog_large_object')").succeeds do
      pending if Fog.mocking?
      Fog::Storage[:openstack].delete_object('foglargeobjecttests', 'fog_large_object')
    end

    tests("#put_object_manifest('foglargeobjecttests', 'large_object_manifest', {'segments_prefix' => 'fog_large_object'})").succeeds do
      pending if Fog.mocking?
      Fog::Storage[:openstack].put_object_manifest(@directory.identity, 'large_object_manifest', {'segments_prefix' => 'fog_large_object'})
    end

    tests("#get_object('foglargeobjecttests', 'large_object_manifest').body").succeeds do
      pending if Fog.mocking?
      Fog::Storage[:openstack].get_object(@directory.identity, 'large_object_manifest').body == ('x' * 6 * 1024 * 1024)
    end
    ...
    tests("returns manifest Etag, computed without manifest file").succeeds do
      ...
      actual = Fog::Storage[:openstack].head_object(@directory.identity, 'large_object_manifest').headers['Etag']
    ...
    tests("#put_object_manifest('foglargeobjecttests2', 'fog_large_object', {'segments_container' => 'foglargeobjecttests', 'segments_prefix' => 'fog_large_object'})").succeeds do
      pending if Fog.mocking?
      Fog::Storage[:openstack].put_object_manifest(@directory2.identity, 'fog_large_object', {'segments_container' => @directory.identity, 'segments_prefix' => 'fog_large_object'})
    end
    ...
    tests("returns manifest Etag, computed without manifest file").succeeds do
      ...
      etags << Digest::MD5.hexdigest('x' * 2 * 1024 * 1024) # => "67b2f816a30e8956149b2d7beb479e51"
      expected = Digest::MD5.hexdigest(etags.join) # => "0b348495a774eaa4d4c4bbf770820f84"
      actual = Fog::Storage[:openstack].head_object(@directory2.identity, 'fog_large_object').headers['Etag']
      actual.gsub('"', '') == expected # actual is returned in quotes "\"0b348495a774eaa4d4c4bbf770820f84"\"
    end
