Shindo.tests('Radosgw::Provisioning | provisioning requests', ['radosgw']) do
  ...
  tests('User creation') do
    ...
    tests('is successful').returns(String) do
      ...
      key_id      = Fog::Radosgw[:provisioning].create_user(email, name).body['key_id']
      key_id.class
    ...
    tests('is successful anonymously').returns(String) do
      ...
      key_id      = Fog::Radosgw[:provisioning].create_user(email, name, :anonymous => true).body['key_id']
      key_id.class
  ...
  tests('User disable') do
    ...
    tests('is successful').returns(200) do
      ...
      Fog::Radosgw[:provisioning].disable_user(key_id).status
  ...
  tests('User enable') do
    ...
    tests('is successful').returns(200) do
      ...
      Fog::Radosgw[:provisioning].disable_user(key_id).status
      Fog::Radosgw[:provisioning].enable_user(key_id).status
  ...
  tests('User granted new key secret') do
    ...
    tests('is successful').returns(true) do
      ...
      Fog::Radosgw[:provisioning].regrant_secret(key_id).status

      # Verify new secret.
      #
      new_key_secret = Fog::Radosgw[:provisioning].get_user(key_id).body['key_secret']
  ...
  tests('User retrieval') do
    ...
    tests('is successful').formats(user_format) do
      ...
      # Get user details.
      #
      Fog::Radosgw[:provisioning].get_user(key_id).body
  ...
  tests('User listing') do
    ...
    tests('successfully lists users containing no disabled users').returns(nil) do
      ...
      # Disable that user.
      #
      Fog::Radosgw[:provisioning].disable_user(key_id)

      # Ensure the list users response does not contain the user that we
      # just created and disabled.
      #
      Fog::Radosgw[:provisioning].list_users(:status => :enabled).body.select { |x| x['Email'] == email }.first
