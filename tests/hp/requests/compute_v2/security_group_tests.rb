Shindo.tests("Fog::Compute::HPV2 | security group requests", ['hp']) do
  ...
  tests('failure') do
    ...
    @security_group =service.security_groups.create(:name => 'fog_security_group_fail', :description => 'tests group')

    tests("duplicate #create_security_group(#{@security_group.name}, #{@security_group.description})").raises(Excon::Errors::BadRequest) do
     service.create_security_group(@security_group.name, @security_group.description)
    end

    tests("#get_security_group(0)").raises(Fog::Compute::HPV2::NotFound) do
     service.get_security_group(0)
    end

    tests("#delete_security_group(0)").raises(Fog::Compute::HPV2::NotFound) do
     service.delete_security_group(0)
    end
