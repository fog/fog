Shindo.tests("Fog::Compute::HPV2 | security_group", ['hp']) do
  # Disabled due to https://github.com/fog/fog/1546
  pending

  service = Fog::Compute.new(:provider => 'HP', :version => :v2)

  model_tests(service.security_groups, {:name => 'foggroupname', :description => 'foggroupdescription'}, true)

  tests("a group with trailing whitespace") do
    @group = service.security_groups.create(:name => "   foggroup with spaces   ", :description => "   fog group desc   ")
    ...
    @other_group = service.security_groups.create(:name => 'other group', :description => 'another group')
